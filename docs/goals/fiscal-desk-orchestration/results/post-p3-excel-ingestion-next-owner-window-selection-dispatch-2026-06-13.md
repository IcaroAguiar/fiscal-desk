# Post P3 Excel Ingestion Next Owner Window Selection Dispatch

Data: 2026-06-13 21:27:21 -03
...
- HEAD observado: `8cab93b feat: add core excel ingestion`
...
O owner window material `post_p3_excel_input_core_ingestion_contract` foi
...
Qual e a proxima owner window mais segura e util depois de integrar o core de
ingestao Excel/XLSX, considerando que a fila material esta vazia e que material
work so pode avancar com owner unico, allowed writes explicitos, checks
proporcionais e review independente?

## Candidatos A Avaliar

Avalie, sem assumir que algum esta automaticamente aprovado:

- proximo recorte para expor entrada Excel no app, se puder ser fatiado sem
  colidir UI, IPC, preload e runtime em owners concorrentes;
- proximo recorte de QA/validacao de ingestao Excel, se for read-only ou
  docs-only e destravar um worker posterior;
...
- proximo recorte de update/release apenas como scope review ou UI blocked
  state, sem update real;
- proximo recorte de hardening de primeira release, se os docs indicarem
  blocker residual apos Excel core;
